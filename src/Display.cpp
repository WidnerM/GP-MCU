...
void LibMain::ClearMCUDisplay()
{
    ...
    sendMidiMessage(GPMidiMessage::makeSysexMessage(GPUtils::hex2binaryString(MCU_CLEAR_BOT)));
    sendMidiMessage(GPMidiMessage::makeSysexMessage(GPUtils::hex2binaryString(MCU_CLEAR_TOP)));
}
...
void LibMain::CleanMCU()
{
    ...
    // shut off all leds
    for (x = 0; x <= 0x76; x++) {
        sendMidiMessage(GPMidiMessage::makeNoteOnMessage(x, 0, 0));
    }

    // clear the knobs
    for (x = 0; x <= 7; x++) {
        sendMidiMessage(GPMidiMessage::makeControlChangeMessage(0x30 + x, 0, 0 ));
    }

    // clear the VU meters on real MCUs
    for (x = 0; x <= 7; x++) {
        sendMidiMessage(GPMidiMessage::makeSysexMessage(GPUtils::hex2binaryString(MCU_CLEAR_VU + (std::string)"0" +
...
void LibMain::DisplayText(uint8_t column, uint8_t row, std::string text, uint8_t maxlength)
{
    ...
    if (column < 8) {
        ...
        hexmessage = MCU_TEXT_HDR + GPUtils::intToHex(row * 0x38 + column * 7) + textToHexString(subtext.substr(0, (maxlength % 7 == 0 ) ? maxlength : maxlength + 7 - maxlength % 7)) + (std::string)" f7";
...
{
    sendMidiMessage(GPMidiMessage::makeNoteOnMessage(button, (value == 0) ? BUTTON_OFF : BUTTON_LIT, 0));
}
...
{
    sendMidiMessage(GPMidiMessage::makeNoteOnMessage(Surface.CommandButtons[FADERS_SELECT], (Surface.TextDisplay == SHOW_FADERS) ? BUTTON_LIT : BUTTON_OFF, 0));
    sendMidiMessage(GPMidiMessage::makeNoteOnMessage(Surface.CommandButtons[KNOBS_SELECT], (Surface.TextDisplay == SHOW_KNOBS) ? BUTTON_LIT : BUTTON_OFF, 0));
    sendMidiMessage(GPMidiMessage::makeNoteOnMessage(Surface.CommandButtons[SONGSRACKS_SELECT], (Surface.TextDisplay == SHOW_SONGS) ? BUTTON_LIT : BUTTON_OFF, 0));
}
